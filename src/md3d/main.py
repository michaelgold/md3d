...
class MacOSInkscapeInstaller(InkscapeInstaller):
    ...
    def install(self):
        ...
        inkscape_url = (
            "https://inkscape.org/gallery/item/44624/Inkscape-1.3.2_arm64.dmg"
        )
        ...
        if response.status_code == 200:
            ...
            subprocess.run(
                [
                    "tar",
                    "-xf",
                    str(source_path / "inkscape.tar.xz"),
                    "-C",
                    str(source_path),
                ],
                check=True,
            ...
            os.chdir(build_path)
            subprocess.run(
                ["cmake", "..", "-DCMAKE_INSTALL_PREFIX=" + str(install_path)],
                check=True,
            )
...
def import_and_position_images(
    ...
    for i, img_path in enumerate(image_paths):
        ...
        bpy.ops.object.light_add(
            type="POINT", radius=1, align="WORLD", location=(0, -6, 3)
        )
...
def import_and_transform_svgs(svg_files, offset_x=3):
    ...
    for i, svg_file in enumerate(svg_files):
        ...
        create_plane = bpy.ops.mesh.primitive_plane_add(
            size=1, enter_editmode=True, align="WORLD"
        )
...
def main(input_md: str):
    ...
    base_dir.mkdir(parents=True, exist_ok=True)
    image_paths, cleaned_md_content = download_images_from_markdown(
        input_md_content, base_dir
    )
